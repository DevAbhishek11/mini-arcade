...
function ticTacToeMove(state: TicTacToeState, seat: Seat, difficulty: BotDifficulty): number {
  ...
  const wins = (index: number, mover: Seat) => {
    const result = ticTacToeEngine.apply({ ...state, turn: mover }, mover, { type: 'place', index }, Date.now());
...
function connectFourMove(state: ConnectFourState, seat: Seat, difficulty: BotDifficulty): number {
  ...
  const wins = (column: number, mover: Seat) => {
    const result = connectFourEngine.apply({ ...state, turn: mover }, mover, { type: 'drop', column }, Date.now());
    return result.ok && result.state.winnerSeat === mover;
  };
...
function gomokuMove(state: GomokuState, seat: Seat, difficulty: BotDifficulty): number {
  ...
  for (const index of list) {
    const score = gomokuScore(state.board, index, seat) + gomokuScore(state.board, index, other(seat)) * defenceWeight;
    if (score > bestScore) {
      bestScore = score;
      best = index;
    }
...
const REVERSI_WEIGHTS = [
  120, -20, 20, 5, 5, 20, -20, 120, -20, -40, -5, -5, -5, -5, -40, -20, 20, -5, 15, 3, 3, 15, -5, 20, 5, -5, 3, 3, 3,
  3, -5, 5, 5, -5, 3, 3, 3, 3, -5, 5, 20, -5, 15, 3, 3, 15, -5, 20, -20, -40, -5, -5, -5, -5, -40, -20, 120, -20, 20,
  5, 5, 20, -20, 120,
];
...
function reversiMove(state: ReversiState, seat: Seat, difficulty: BotDifficulty): number | null {
  ...
  for (const index of legal) {
    ...
    const score =
      difficulty === 'brutal'
        ? positional * 2 - mobility * 6 + discs
        : positional + discs * 2 - mobility;
    if (score > bestScore) {
      bestScore = score;
      best = index;
    }
...
export function decide(
  ...
  switch (gameId) {
    ...
    case 'dots-and-boxes': {
      ...
      if (typed.turn !== seat) return null;
      return { action: { type: 'draw', edge: dotsMove(typed, seat, difficulty) }, delayMs: thinkTime(difficulty) };
    }
    case 'pong': {
